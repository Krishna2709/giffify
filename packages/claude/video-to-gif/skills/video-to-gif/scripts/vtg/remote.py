...
# SEC-015 redaction lives in vtg.redaction so the CLI's crash sink can redact
# without importing this module's http.client/ssl/email stack; re-exported here
# so every existing `remote.redact_url` / `remote.redact_message_urls` reference
# (including the security tests) keeps resolving.
from .redaction import _URL_IN_TEXT, redact_message_urls, redact_url  # noqa: F401
